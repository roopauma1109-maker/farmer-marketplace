...
export const RegisterPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.name.trim() || !formData.mobile.trim() || !formData.email.trim() || !formData.password) {
  ...
  return (
    <div className="container" style={{ maxWidth: '540px', padding: '3rem 1.5rem' }}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="form-group" style={{ marginBottom: '1.5rem' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
              <button
                type="button"
                className={`btn ${formData.role === 'farmer' ? 'btn-primary' : 'btn-secondary'}`}
                onClick={() => setFormData({ ...formData, role: 'farmer' })}
                style={{ padding: '0.85rem', display: 'flex', flexDirection: 'column', gap: '0.25rem' }}
              >
                <Sprout size={20} />
                <span style={{ fontSize: '0.9rem' }}>{t('roleFarmer')}</span>
              </button>
              <button
                type="button"
              ...
              >
                <ShoppingBag size={20} />
                <span style={{ fontSize: '0.9rem' }}>{t('roleBuyer')}</span>
              </button>
            </div>
          </div>
          ...
          >
            {submitting ? t('creatingAccount') : t('navRegister')}
          </button>
        </form>
